/**
 * s28 tour 18 — Capture du CANVAS seul (zoom plein écran),
 * pour voir précisément les polygones par étage.
 */
import { chromium, type Browser, type Page } from "playwright";

const PROJECT_ID = process.env.VS_E2E_PROJECT_ID || process.argv[2];
if (!PROJECT_ID) {
  console.error("Usage: VS_E2E_PROJECT_ID=xxx tsx scripts/s28-tour18-canvas-only.ts");
  process.exit(1);
}
...
async function captureFloor(page: Page, floor: number) {
  ...
  // Cible directement le canvas
  const canvas = page.locator("canvas").first();
  const box = await canvas.boundingBox();
  if (!box) {
    console.warn(`[Floor ${floor}] canvas not found`);
    return;
  }
  const path = `${SCREENSHOT_DIR}/s28-tour18-floor-${floor}.png`;
  await canvas.screenshot({ path });
  console.log(`[Floor ${floor}] canvas saved: ${path} (${box.width.toFixed(0)}x${box.height.toFixed(0)})`);
}
