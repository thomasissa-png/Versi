...
async function main() {
  ...
  // Box d'intérêt : zone autour des outliers SDE F3 (px 1730-1900, y 1610-1700)
  const x0 = 1700, x1 = 1920, y0 = 1600, y1 = 1750;
  console.log(`Walls dans box [${x0}-${x1}, ${y0}-${y1}] :`);
  let count = 0;
  for (const w of allWalls) {
    ...
    count++;
    if (count <= 30) {
      console.log(`  [${w.src}] (${w.x1.toFixed(0)},${w.y1.toFixed(0)})→(${w.x2.toFixed(0)},${w.y2.toFixed(0)}) len=${Math.hypot(w.x2-w.x1,w.y2-w.y1).toFixed(0)}`);
    }
  }
  console.log(`Total : ${count} walls dans la zone`);
  await pool.end();
}
