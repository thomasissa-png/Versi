/**
 * Lit s27-vs-autopilot-best.json (produit par autopilot-grid.ts)
 * et rend les overlays des 4 plans Muguets avec les meilleurs params.
 */
...
async function main() {
  const bestRaw = await readFile(join(OUT, "s27-vs-autopilot-best.json"), "utf8");
  const best = JSON.parse(bestRaw) as { opts: ColorMaskOptions };
  console.log("Best opts:", JSON.stringify(best.opts, null, 2));
  for (const p of PLANS) {
    const png = await rasterize(p.file);
    const r = await extractLotsByColorMask(png, best.opts);
    const poly = r.polygons[0] ?? [];
    console.log(`  ${p.name}: ${poly.length} sommets`);
    const overlay = await renderRaw(png, poly);
    const outPath = join(OUT, `s27-vs-best-${p.name}.png`);
