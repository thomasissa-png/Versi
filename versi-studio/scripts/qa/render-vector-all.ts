/**
 * Render extracteur vectoriel sur les 4 plans Muguets.
 */
...
async function processOne(file: string, name: string) {
  const buf = await readFile(join(process.cwd(), "reference-existant", "plans-test", file));
  const r = await extractLotVector(buf, { scale: 3 });
  console.log(`${name}: ${r.wallSegments.length} wall segments → polygon ${r.polygon.length} sommets`);
  ...
  const overlay = await renderRaw(png, r.polygon);
  const outPath = join(OUT, `s27-vs-final-${name}.png`);
