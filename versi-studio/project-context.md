...
| copywriter | 2026-04-16 | docs/reviews/autopilot/vs-step0-copy-v3.md | Re-audit v3 post-fixes F1-F11. Score 9/10 — GO. Les 4 corrections R1-R4 de v2 toutes appliquées : R1 tutoiement corrigé (formulation neutre), R2 "créez" au lieu de "générez", R3 `draft: "En cours"`, R4 "Chargement de vos opérations…". Grep : 0 tutoiement, 0 "générer", 0 "IA" dans l'interface. Erreurs fetch 404/503/fallback différenciées et conformes. Point résiduel mineur : ProjectCard sans signal textuel d'entrée ("Continuer →"). | Les 4 corrections étaient les violations bloquantes identifiées en v2 (7.5/10). Passage à 9/10 par correction complète sans régression. |
...
| product-manager | 2026-04-16 | docs/reviews/autopilot/vs-step0-pm-v3.md | Re-audit v3 post-11-fixes. Score 9/10 — GO. Les 5 points résiduels P1-P5 de v2 tous vérifiés : P1 vouvoyement+wording PASS, P2 surface min=9 max=5000 step=1 PASS, P3 truncate PASS, P4 analytics 4 stubs PASS, P5 US-VS-00 dans specs PASS (fichier dans repo parent). 4 points résiduels mineurs identifiés : R1 aria-busy forme nominale (cosmétique), R2 discordance specs US-VS-00 surface 10-9999→9-5000 (à corriger dans specs), R3 analytics timing/propriétés (backlog Umami), R4 focus-visible bouton Annuler (1 ligne). G27 Tests E2E = FAIL structurel attendu — @qa Phase 2d non lancée. | v2 était 7.5/10 car P1-P5 non vérifiables (code pas encore lu). v3 lit le code ligne par ligne — tous les 11 fixes F1-F11 confirmés PASS. Score 9/10 et pas 10/10 car G27 (gate REQUIS) FAIL — aucun test E2E produit. |
| ux | 2026-04-19 | docs/ux/s23-etape1-upload-layout-rework.md | Option A (box shrink) recommandée : DropZone passe en mode compact (~52px) dès le 1er upload via prop `compact`. Cause racine : `min-h-[200px]` + `p-4xl` statiques poussent la grille de plans sous le fold (~504px avant grille sur un viewport 900px). 2 fichiers à modifier : DropZone.tsx (prop compact) + upload/page.tsx (condition plans.length > 0). | Option B (side-by-side) écartée : complexité layout + conflit avec stepper latéral déjà présent. Option C (inversion ordre) écartée : counter-intuitif sur l'état vide. |
| ux | 2026-05-07 | docs/ux/s33-reaudit-ux-vague2.md | Re-audit ciblé vague 2 : 8 fixes sur 8 PASS (B-1 Stepper étape 4, B-2 Undo/Redo toolbar top-right, B-3 Empty state canvas, B-4 Checklist génération, F-1 Bulk tout confirmer, F-2 Toast 8s, F-3 Toast undo suppression, F-4 Sélecteur visuels footer). Score 7/10 → 9/10. 1 P1 résiduel : toast chat architecte 5s (à migrer vers UNDO_WINDOW_MS = 8000). | Audit read-only sur commits ca4c1e2/1707c94/10af1f8. Tous les P0 et P1 de la Phase 1 sont résolus. Le 10/10 strict requiert 1 fix supplémentaire (10 min) + test persona bout-en-bout. |
