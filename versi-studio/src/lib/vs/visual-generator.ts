...
export function buildVisualPrompt(
  ...
  const structuralRule = hasTransformations
    ...
    : `1. KEEP all structural elements EXACTLY as they are: walls, windows, doors, openings, bay windows, ceiling, floor shape, room proportions. Do NOT move, add, or remove any window, door or opening. Specifically, preserve every existing door, window and opening visible in the source photo with its exact position, size, opening direction, frame and sill. The doors visible in the source must remain in the output, even if partially hidden by furniture. Do not invent doors, windows or openings that are not visible in the source.`;
