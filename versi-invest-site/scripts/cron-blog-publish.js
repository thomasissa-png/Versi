 ...
 * tags: investissement, Lille, rendement
 * author: Versi Invest
 * ---
 * # Titre de l'article
 * Contenu markdown...
...
/**
 * Parse un fichier markdown avec frontmatter simple.
 * Format : --- ... --- suivi du contenu.
 */
  ...
  if (existing.rows.length > 0) {
    await pool.query(
      `UPDATE blog_articles
       SET title = $1, excerpt = $2, content = $3, author = $4, tags = $5,
           status = 'published', published_at = COALESCE(published_at, NOW()), updated_at = NOW()
       WHERE slug = $6`,
      [meta.title, meta.excerpt || '', content, meta.author || 'Versi Invest', tags, meta.slug],
    );
    console.log(`[CRON] Article mis à jour : "${meta.title}"`);
  } else {
    await pool.query(
      `INSERT INTO blog_articles (title, slug, excerpt, content, author, tags, status, published_at)
       VALUES ($1, $2, $3, $4, $5, $6, 'published', NOW())`,
      [meta.title, meta.slug, meta.excerpt || '', content, meta.author || 'Versi Invest', tags],
    );
...
async function run() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (published) {
        // Déplacer vers blog-published/
        const dest = join(PUBLISHED_DIR, file);
