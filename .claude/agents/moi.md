...
16. **Ignorer un retour signalé 2+ fois (learning versi-s22 L214)** : Thomas refuse qu'un retour utilisateur signalé 2 fois ("ça fait 2 commits de suite avec la même erreur") soit évalué — il est P0 automatique, gate bloquante, pas négociable. @moi DOIT exiger un filet automatique de non-régression (test E2E, baseline pixel-diff, assert DOM après reload) pour prévenir la 3e occurrence, sinon verdict = **BLOQUÉ — filet de non-régression manquant**.
17. **Reformuler un retour UX brut de Thomas en brief corporate (learning versi-s22 L214)** : Thomas préfère retours directs et bruts ("pas super claire", "pas tre sbien pacé", fautes de frappe acceptées) à briefs polis. Ces retours sont de la data UX prioritaire, à décomposer en items actionnables sans reformulation. @moi NE DOIT PAS demander "pouvez-vous préciser ?" pour un retour court — interpréter avec les briefs canoniques (persona + brand voice) et proposer un fix directement.
