...
9. **Max 8 sections par livrable (s29).** Empiriquement : briefs > 8 sections déclenchent timeout API stream même avec skeleton-first. Si la mission demande > 8 sections, découper en 2 livrables séquentiels (ex : `specs-part1.md` sections 1-7 + `specs-part2.md` sections 8-14). Skeleton-first reste obligatoire dans chaque fichier. Pattern empirique s29 : 4 agents (PM/UX/IA/QA) ont timeouté en Phase 1+2 Étape 4 v2 sur briefs 9-12 sections — fix par découpage.
10. **Skeleton-first explicite (s29).** Le skeleton n'est pas une recommandation, c'est une règle. Au premier Write : écrire UNIQUEMENT les titres `## Section X` + 2-3 lignes de résumé par section + bloc TODO inline. Puis 1 Edit par section pour remplir (max 80 L par Edit). Cette discipline évite la longue rédaction continue qui dépasse les limites idle stream API.
...
### Règles s27.2 (pipeline + vérification pre-claim, propagées s28)

- **Wire prod ≠ wire diagnostic — toujours grep call-sites avant clôture pipeline.** Avant de fermer un changement « pipeline », `grep -rn "OldFunction\|NewFunction" src/app/api` pour confirmer que TOUS les call-sites prod sont migrés. Bug s27.2 : route `/api/vs/diagnostics/pipeline-new` modifiée mais route prod `/api/vs/projects/[id]/extract` utilisait encore l'ancien pipeline → Thomas voyait « la même erreur après update ». Pre-commit checklist pipeline obligatoire : (1) module modifié, (2) route diagnostic migrée, (3) route prod migrée — les 3 confirmées par grep avant push.
- **Itérer sans s'arrêter MAIS pivoter au plateau.** Lecture combinée des 13 verbatims Thomas s27 : itérer SANS pause questions inutiles MAIS être HONNÊTE sur les plafonds. Quand 2-3 itérations consécutives donnent un gain marginal (<5%) sur la métrique cible, STOP itération horizontale → explorer approche orthogonale (bitmap → vector, prompt → post-process déterministe, etc.). Annoncer le pivot honnêtement : « plateau X atteint à itération N, je pivote sur Y ». Validé empiriquement s27 : 9 itérations bitmap plateau IoU 0.80 → pivot vectoriel = pixel-perfect en 1 commit (`670efba`).
- **Vérification rigoureuse pre-claim (P1, anti « as-tu seulement vérifié ? »).** Avant tout livrable « final » ou claim « parfait », faire un compte-rendu de vérification bord par bord / point par point / cas par cas. Format obligatoire : tableau ou liste avec verdict (✓/✗) sur chaque sous-élément. Si rien n'a été vérifié → écrire « LIVRÉ NON-VÉRIFIÉ — à valider par Thomas » plutôt que « RÉSULTAT FINAL ». Trigger : quand Thomas écrit « as-tu seulement vérifié ? » → on n'a pas comparé rigoureusement avec la référence. Source s27.2.

### Règles s28 (audit visuel orchestrator + pivot 5-tours, propagées s29)

- **Audit visuel orchestrator strict — gate pre-claim.** S28 : 21+ tours d'audit visuel orchestrator faux par comparaison superficielle PDF/screenshot. Verdict score / GO / final sur livrable visuel exige : (1) lire screenshot ET référence côte à côte dans la même réponse (pas de souvenir), (2) tableau ✓/✗ par sous-élément, (3) si non vérifié → « NON-VÉRIFIÉ — à valider par Thomas » plutôt que « X/10 ». Tout score chiffré DOIT être accompagné du tableau de critères qui le justifient. Anti-pattern : « ça paraît mieux », « ~8/10 visuel » sans tableau. Source s28.
- **Pivot orthogonal après plateau 5 tours (renforce iterate-vs-pivot-honest s27.2).** Gate explicite : **après 5 itérations consécutives sans gain net (<5% sur la métrique cible) sur la même approche algorithmique, l'agent DOIT proposer un pivot architectural orthogonal, PAS insister.** S28 a démontré : 7 itérations BFS+post-traitement plateau 17/20, puis pivot bbox-from-walls (tour 18) = saut qualitatif. Pivots tour 29 (invariant touche), tour 32 (override cap PDF) ont chacun débloqué le plateau. Format obligatoire au tour 5 plateau : « **PLATEAU DÉTECTÉ** — 5 itérations sans gain >5%. Approche actuelle : X. Pivot proposé : Y (axe orthogonal Z). Annonce honnête plutôt que tour 6 incrémental. » Source s28 (renforce s27.2).

### Règles s32 (audit exhaustif après 2 patches réactifs, propagées s33)

- **Gate « 2 patches réactifs sur même domaine = trigger audit exhaustif ».** Source s32 déploiement Cloud Run : 4 patches successifs avant que Thomas exige l'audit (« je ne suis pas là pour débugger »). À la **2ème itération réactive sur un même domaine** (déploiement, pipeline IA, parsing PDF, extraction OCR, migration, etc.), tout agent DOIT stopper les patches symptomatiques et passer en audit exhaustif : inventaire complet du domaine + test E2E scénario UTILISATEUR réel (pas test, utilisateur) + vérification wire-up de chaque élément critique + risques résiduels documentés + annonce explicite au fondateur (« 2 patches réactifs détectés sur [domaine], je passe en audit exhaustif »). Anti-pattern : enchaîner 3+ patches en espérant que le suivant tienne. Apparenté `iterate-vs-pivot-honest` s27.2 mais sur axe debug/deploy plutôt qu'algo. Voir `orchestrator.md` Règles s32 pour le format détaillé.
