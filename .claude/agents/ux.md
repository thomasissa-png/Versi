...
### Préférences fondateur UX — patterns versi-s22

- **Découvrabilité UI : feature invisible = feature inexistante** — Quand Thomas dit "je ne vois pas la feature X", vérifier la DÉCOUVRABILITÉ UI (pas juste l'existence du code). Pattern obligatoire : boutons UI permanents (pas conditionnels), visibles dès l'arrivée sur la page. Source versi-s22 : Thomas a demandé 3+ fois le zoom Étape 2 (boutons +/-) avant qu'il soit livré (le code wheel+pan existait déjà mais sans boutons UI). Règle : pour TOUT geste avancé (wheel zoom, pan, drag), prévoir AUSSI un fallback bouton UI visible.
- **Undo/redo obligatoire sur tout canvas éditeur** — Pattern Figma/Miro : Ctrl+Z + Ctrl+Shift+Z + boutons UI Undo/Redo dans la toolbar. Stack ≥ 50 ops. Aucune "édition non réversible" acceptable. À implémenter sur lots, rooms, visuels, plans, schémas, annotations.
- **Minimum de clics : 1 bouton qui fait 2 actions > 2 boutons séparés** — Thomas privilégie toujours le minimum de clics. Un flux "valide + avance" en 1 clic > flux "valide puis avance" en 2 clics, même si techniquement les 2 actions sont distinctes. Bouton contextuel : si état A → fait les 2 actions ; si état B → fait directement la suite. Source versi-s22 : LotPanel.tsx bouton "Valider et passer aux pièces" répété 3 fois avant implémentation.
- **Comparateur avant/après obligatoire sur génération IA** — Pour toute feature "génération IA" où Thomas montrera le résultat à un tiers (client, prospect), afficher un comparateur avant/après par défaut (pas juste le résultat seul). Layout CSS Grid 2 colonnes 50/50 desktop / stack mobile. Labels "Avant (photo actuelle)" / "Après (visuel IA)". Lightbox + carousel multi-versions. Source versi-s22 : VisualResult.tsx — pattern standard immobilier.
- **Pas de modification silencieuse du workflow métier** — Quand Thomas a défini un parcours (ex : Étape 1 → 2 → 3 → 4 avec ordre + sémantique précis), NE JAMAIS le modifier silencieusement (réordonner, fusionner, sauter une étape, changer le sens d'un bouton). Si un agent veut suggérer une modification : la proposer EXPLICITEMENT dans le handoff avec rationale, ne jamais l'appliquer sans validation. Source versi-s22 : un agent avait silencieusement modifié l'ordre des actions Étape 3 — détecté par Thomas, rollback nécessaire.
