...
11. **Objectif qualité : 100% gates PASS.** Chaque livrable sera évalué par @reviewer via 34 gates binaires G1-G34 (PASS/FAIL) réparties en BLOQUANT et REQUIS. Le seuil de validation est : 100% gates BLOQUANT PASS + 100% gates REQUIS PASS. Viser l'excellence dès la première passe pour éviter les itérations correctives
...
14. **Vérification de branche obligatoire en début de session.** En début de CHAQUE session, l'agent DOIT : (1) vérifier `git branch` et confirmer qu'il est sur la bonne branche (référencée dans le mémo de reprise de `project-context.md`), (2) vérifier un marqueur de contenu validé (ex: `Grep "Trois fondateurs" src/src/components/Team.jsx`), (3) si mismatch → STOP immédiat, signaler au fondateur. Ne JAMAIS travailler sans cette vérification. Cause : session versi-s3 entière perdue par travail sur une branche régressive.
15. **Zéro mention de concurrent par nom dans les livrables client-facing.** Ne JAMAIS mentionner de concurrent par nom dans le code frontend, le copy, le contenu marketing, le SEO ou tout contenu visible par l'utilisateur final. Utiliser des catégories génériques ("freelance marketing", "outil avec templates", "plateforme SaaS"). Exception : les livrables internes (benchmarks concurrentiels, audits stratégiques, analyses de marché) DOIVENT nommer les concurrents pour être actionnables.
16. **Anti-complaisance obligatoire dans les audits (learning versi-s10).** TOUS les briefs d'audit agent DOIVENT inclure : "Note HONNÊTE. Un 7 est correct pour un premier site. Ne gonfle pas les notes. Corrections EXACTES si < 9." Sans ces instructions, le biais de complaisance IA est systématique (~2 points de surcotation mesurés). Les audits DOIVENT vérifier le rendu visuel réel (nombre de lignes à la taille de police réelle), pas juste le nombre de caractères.
17. **Zéro promesse non-systématique en position H1/surtitre/meta (learning versi-s10).** Ne JAMAIS mettre en position H1, surtitre ou meta une caractéristique qui n'est pas 100% systématique. Si c'est "parfois" ou "souvent" → FAQ ou corps de texte, pas en promesse principale. Exemple : "off-market" ne peut être en H1 que si 100% des biens sont off-market.
18. **Process métier = vérité fondateur, jamais invention agent (learning versi-s10).** Les process métier (étapes d'un workflow, pipeline opérationnel) DOIVENT être validés par le fondateur AVANT implémentation. Les agents ne doivent JAMAIS inventer un workflow métier basé sur les specs — ils doivent demander "quel est votre vrai processus étape par étape ?".
19. **Zéro anglicisme en copy client-facing (learning versi-s15, P0 bloquant).** Liste noire obligatoire en surface utilisateur (UI, micro-copy, ARIA labels, specs user-facing, tests E2E visuels, **messages d'erreur API retournés au client** — `NextResponse.json({ error: "..."})` affichés via `setError(json.error)` ou équivalent) : `upload` / `uploader` / `uploadé` / `uploadez` → `déposer` / `déposé` / `déposez` ; `download` / `downloader` → `télécharger` ; `feedback` → `retour` (utilisateur) ; `meeting` → `réunion` / `rendez-vous` ; `forwarder` → `transférer` / `transmettre`. Exceptions tolérées (usage FR courant, pas d'équivalent idiomatique) : `scroll` / `scroller`, `clic` / `cliquer`, `email`, `site`, `login` / `logout`, `cookie`. N'affecte PAS les commentaires code, identifiants techniques, noms de variables/fonctions. Les agents @copywriter, @ux, @seo, @fullstack DOIVENT vérifier à chaque livrable, **y compris les messages d'erreur API**. Gate G33 (BLOQUANT) contrôle. Voir `docs/founder-preferences.md` section Langue française.
20. **Zéro collision custom property `@theme` Tailwind v4 avec scale builtin (learning versi-s15, P0 bloquant).** Sur Tailwind v4 `@theme {}`, ne JAMAIS nommer une custom property avec un préfixe qui matche une scale Tailwind builtin : `--spacing-*` collide avec `max-w-*` (ex : `--spacing-md: 16px` écrase `max-w-md` de 448px → 16px, impact systémique). Préfixes à proscrire : `--spacing-`, `--sizing-`, `--rounded-`, `--leading-`, `--tracking-`. Alternatives : préfixer en `--space-*`, `--size-*`, `--radius-*`, `--lh-*`, `--ls-*` OU utiliser valeurs arbitrary `max-w-[28rem]`. Gate G34 (BLOQUANT) contrôle.
21. **Tests exécutés, pas juste écrits (learning versi-s21, P1).** Tout verdict GO PRODUCTION @moi DOIT être précédé de la preuve d'exécution des tests dans la session : sortie console visible de `npx tsc --noEmit` (0 erreur ou liste exacte), `npx vitest run` (X/X PASS + durée), `npx playwright test` (X/X PASS + durée), `npm run lint` (0 erreur production, warnings et erreurs legacy `reference-existant/` tolérés mais documentés). Les audits textuels (code review, @qa audit) sont NÉCESSAIRES mais PAS suffisants. L'orchestrateur DOIT installer `node_modules` dès le début de session si absent (`npm install` en background Phase 0 — n'ajoute pas de Task producteur). @qa DOIT ajouter `vitest` + `@vitest/ui` comme `devDependency` dans `package.json` lors de la création de `tests/unit/*.test.ts`. @moi DOIT refuser tout GO PRODUCTION sans sortie console visible. Voir `docs/founder-preferences.md` section Tests exécutés.
22. **Anti-clôture prématurée de session (learning versi-s21, P1).** L'orchestrateur NE DOIT JAMAIS clôturer une session tant que le brief initial contient des priorités non-traitées, même si la priorité principale (P1) a été validée GO PRODUCTION. GO PRODUCTION sur P1 = milestone, pas terminal de session. Avant toute proposition de clôture : (1) Grep `P[1-9]` dans le brief initial de la session, (2) checklist statut de chaque priorité (traitée / reportée explicitement avec validation fondateur / en cours), (3) si au moins une priorité non couverte → continuer la session, NE PAS proposer clôture. La gate finale @moi valide le livrable PRINCIPAL, pas la session. Voir `docs/founder-preferences.md` section Clôture de session.
...
2. **Audit complet par @reviewer** (en fin de run, Étape 7) : exécuter les 34 gates (BLOQUANT + REQUIS + CONDITIONNEL) via Grep/Read/comparaison — pas de jugement subjectif. Boucle d'itération si besoin (max 3 passes). Les verdicts sont inscrits dans le tableau "Performance des agents".

### Les 34 gates binaires (PASS/FAIL)
...
| G33 | Zéro anglicisme en copy client-facing (learning versi-s15, périmètre élargi versi-s16) | BLOQUANT | Pour code/copy/specs/tests visuels **ET messages d'erreur API retournés au client** (`NextResponse.json({ error: "..."})` affichés via `setError(json.error)`) : Grep `upload\|uploader\|uploadé\|uploadez\|download\|downloader\|feedback\|meeting\|forwarder` en surface utilisateur (JSX rendu, strings UI, labels ARIA, tests `getByText`, **strings dans les routes `route.ts` retournées au client**). Si 1+ occurrence hors commentaires code ou identifiants techniques → FAIL. Remplacement obligatoire : `déposer\|télécharger\|retour\|réunion\|transférer`. Exceptions tolérées : `scroll, clic, email, site, login, cookie`. Voir règle n°19 CLAUDE.md |
| G34 | Zéro collision custom property `@theme` Tailwind v4 avec scale builtin (learning versi-s15) | BLOQUANT | Pour CSS `@theme {}` (typiquement `globals.css` / `app.css`) : Grep `--spacing-\|--sizing-\|--rounded-\|--leading-\|--tracking-` dans les blocs `@theme`. Si 1+ définition collide avec une scale Tailwind builtin (`spacing`, `max-w`, `rounded`, `leading`, `tracking`) → FAIL. Voir règle n°20 CLAUDE.md. Test de vérification : `max-w-md` rendu < 400px dans le navigateur = symptôme d'une collision `--spacing-md` |
...
**Règle (reviewer)** : en fin de run, exécuter les 34 gates sur chaque livrable. Tout livrable avec 1+ gate BLOQUANT ou REQUIS FAIL déclenche une boucle d'itération (max 3 passes). Voir `orchestrator.md` Étape 7.
