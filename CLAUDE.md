...
19. **Zéro anglicisme en copy client-facing (learning versi-s15, P0 bloquant).** Liste noire obligatoire en surface utilisateur (UI, micro-copy, ARIA labels, specs user-facing, tests E2E visuels) : `upload` / `uploader` / `uploadé` / `uploadez` → `déposer` / `déposé` / `déposez` ; `download` / `downloader` → `télécharger` ; `feedback` → `retour` (utilisateur) ; `meeting` → `réunion` / `rendez-vous` ; `forwarder` → `transférer` / `transmettre`. Exceptions tolérées (usage FR courant, pas d'équivalent idiomatique) : `scroll` / `scroller`, `clic` / `cliquer`, `email`, `site`, `login` / `logout`, `cookie`. N'affecte PAS les commentaires code, identifiants techniques, noms de variables/fonctions. Les agents @copywriter, @ux, @seo, @fullstack DOIVENT vérifier à chaque livrable. Gate G33 (BLOQUANT) contrôle. Voir `docs/founder-preferences.md` section Langue française.
