...
## Règles custom Versi (pointeurs — cap commandement n°8)

- **Workflow d'audit visuel des générations IA** (règle critique, préserve le pipeline Yann/Lucas/Camille) : voir [`docs/workflows/audit-visuel-generations.md`](docs/workflows/audit-visuel-generations.md). Ne JAMAIS lancer un agent d'audit sans pré-fetch.
- **Règles complémentaires actives s22/s23/s24** (reality check E2E, découvrabilité UI, pixel-parfait, agrégats raffinés, sync représentations, fail-fast, mot pivot métier, tsc sans filtre) : voir [`docs/claude-md-archive.md`](docs/claude-md-archive.md). Source de vérité tant que non-promues dans les agents.
- **Learnings récents** (table statut + TTL) : voir [`docs/lessons-learned.md`](docs/lessons-learned.md) — cap 80 L.
- **Mémos de reprise sessions** (5 dernières actives, anciennes archivées) : `project-context.md` + [`project-context-archive.md`](project-context-archive.md).
- **Préférences fondateur** : [`docs/founder-preferences.md`](docs/founder-preferences.md).
