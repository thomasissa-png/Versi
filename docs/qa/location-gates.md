...
| # | Nom | Méthode | PASS | FAIL |
|---|---|---|---|---|
| GL-5 | Transport complet — transport principal + accès gare | Grep | Au minimum : 1 station du transport principal de la ville (métro, tramway, RER, bus structurant selon la ville) nommée avec ligne(s) + mention de l'accès aux gares principales. Mentionner l'accès gare uniquement si le trajet porte-à-porte en TC est <= 20 min (vérifié Moovit/Google Maps) — sinon N/A | Aucune station de transport en commun nommée |
...
| GL-8 | Santé — établissement de santé identifié | Grep | Au moins 1 établissement de santé nommé (hôpital, clinique, CHU, ou cabinet médical/médecin généraliste si quartier résidentiel) avec sa spécificité si pertinent (urgences, pédiatrie). Pour les biens 3+ pièces (cible famille), mentionner un pédiatre ou service pédiatrique si disponible dans le quartier | Aucune mention de santé |
...
| GL-10 | Crèches mentionnées si famille cible | Grep (conditionnel — le vérificateur lit le champ `rooms` du bien) | Si le bien a 3+ pièces (famille cible), au moins 1 crèche nommée (municipale, privée, ou micro-crèche). N/A pour T1/studios | Bien 3+ pièces sans mention de crèche |
...
| GL-12 | Longueur maîtrisée | Compteur | `nearby_transport` : 25 à 80 mots. `nearby_amenities` : 50 à 120 mots. Un texte trop long déséquilibre la fiche bien | `nearby_transport` < 25 ou > 80 mots, OU `nearby_amenities` < 50 ou > 120 mots |
...
2. Accès aux gares principales (direct ou correspondance). N/A si trajet porte-à-porte en TC > 20 min
...
idéalement desservi, parfaitement desservi, bien desservi, toutes commodités,
à portée de main, environnement verdoyant, quartier dynamique
```
