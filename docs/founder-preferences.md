...
### Itérer sans s'arrêter MAIS pivoter quand plafond atteint (s27)
Verbatim Thomas s27 (13 fois sur la même session) : *« Itère en autopilote jusqu'au résultat parfait ne t'arrête pas »* + *« encore une fois je veux juste que tu testes toutes les approches jusqu'à trouver une qui marche, et me presenter le resultat final »*. Lecture combinée : itérer SANS s'arrêter (pas de pause questions inutiles) MAIS être HONNÊTE sur les plafonds. Ne pas faire semblant que la 10e itération va finir par marcher si la précédente a déjà plateau. Validé empiriquement s27 : 9 itérations bitmap plateau IoU 0.80 → pivot vectoriel = pixel-perfect en 1 commit.
**Règle orchestrator + ia + fullstack** : quand 2-3 itérations consécutives donnent un gain marginal (<5%) sur la métrique cible, STOP l'itération horizontale et explorer une approche orthogonale (changement de paradigme : bitmap → vector, prompt → post-process déterministe, etc.). Annoncer le pivot honnêtement : « plateau X atteint, je pivote sur Y ». Pas de « peut-être que la 10e marchera ».

### Pattern UI buttons unifiés > formulations marketing (s27)
Verbatim Thomas s27 : *« francembt c'est nul. Pourquoi ne pas faire quelque chose de clair type Ajouter un lot (forme rectangulaire) / Ajouter un lot (forme libre) ? »*. Rejet explicite des formulations marketing « jolies » (« Lot rapide / Lot sur mesure ») au profit du pattern unifié `Action principale (précision)`. La différenciation passe par les parenthèses, l'action reste identique. Vocab pro sobre, pas de marketing fluffy.
**Règle copywriter + ux + design** : pour 2 boutons UI similaires (même action, variants), utiliser le pattern `Verbe + Objet (variant entre parenthèses)` plutôt que des adjectifs imagés. Ex : `Ajouter un lot (forme rectangulaire)` / `Ajouter un lot (forme libre)`, `Exporter (CSV)` / `Exporter (PDF)`, `Filtrer (par date)` / `Filtrer (par montant)`. Cohérence visuelle (mêmes 3 premiers mots) + différenciation lisible factuelle. Pas de « rapide / sur mesure / standard / express ».

### "As-tu seulement vérifié ?" = trigger d'auto-critique honnête (s27)
Quand Thomas écrit *« as tu seulement vérifié ? »* après un claim « RÉSULTAT FINAL / PARFAIT », il a détecté qu'on n'a PAS comparé rigoureusement avec la référence. Pattern à adopter : avant tout claim « parfait/final/livré », vérifier RIGOUREUSEMENT (zoom sur chaque bord du résultat vs référence) ET formuler l'auto-critique honnête (ex : « RDC bord haut OK / R+1 bord droit déborde 30px / R+2 ... »). Une auto-critique honnête vaut mieux qu'un claim sans vérification.
**Règle reviewer + qa + tous agents producteurs** : avant tout livrable « final » ou claim « parfait », faire un compte-rendu de vérification bord par bord / point par point / cas par cas. Format obligatoire : tableau ou liste avec verdict (✓/✗) sur chaque sous-élément. Si rien n'a été vérifié → dire « LIVRÉ NON-VÉRIFIÉ — à valider par Thomas » plutôt que « RÉSULTAT FINAL ».

### Pas de blocage technique sur dépense IA — système de crédits V3 (s29)
Verbatim Thomas s29 : *« Nonon pas de blocage d'argent c'est ok. Continue. Chaque visuel utilisera des crédits mais on verra plus tard cette partie. »* Sur Étape 4 v2 Visuels, persona avait recommandé un circuit breaker $5/projet → REJETÉ. Pattern : afficher un compteur de coût indicatif est OK, bloquer techniquement la génération sur seuil ne l'est pas. Le système de crédits utilisateur (cap business) est une feature V3 distincte.
**Règle product-manager + fullstack** : ne JAMAIS proposer de circuit breaker / cap technique / blocage budgétaire sur dépenses IA sans demande explicite Thomas. Affichages de coût indicatifs OK pour transparence. Le cap business est géré au niveau crédits utilisateur, pas au niveau techno.

### Validation persona AVANT implémentation sur refontes UX (s29)
Verbatim Thomas s29 : *« Pour la validation demande à marchand de bien persona de me donner son avis. »* Lors de la refonte Étape 4 v2 Visuels, Thomas a explicitement intercalé une étape de validation persona entre Phase 2 (specs+wireframes+pipeline) et Phase 3 (implémentation). Le persona @testeur-persona-thomas-marchand a flaggé un FAIL GP5 (mobile placement tactile) qui a infléchi Phase 3 (investissement spécifique tactile vs simple "responsive"). Pattern : **sur toute refonte UX structurelle, lancer obligatoirement testeur-persona avant Phase 3 implémentation**. Le persona valide l'usage terrain réel, pas la stratégie produit.
**Règle orchestrator** : workflow refonte = Phase 1 specs → Phase 2 design+ia → **Phase 2.5 validation persona (gate)** → Phase 3 implémentation. Ne pas lancer Phase 3 sans avoir le verdict persona en main, même si les arbitrages produit sont déjà tranchés.


### "Je ne suis pas là pour débugger" — audit exhaustif au lieu de patches réactifs (s32)
Verbatim Thomas s32 (deploiement Cloud Run cassé 4 fois consécutives) : *« je suis abasourdi. Fais une vraie verif. Je suis pas là pour débugger »*. Après 4 patches réactifs deploy (`output: standalone`, path dynamique, HOSTNAME, pdf.worker), Thomas exige un audit exhaustif au lieu de continuer le whack-a-mole. L'audit a révélé 5 trous supplémentaires (heic-convert, libheif-js, exifr, sharp + @img prebuilds) qui auraient cassé en cascade.
**Règle orchestrator + infrastructure + fullstack** : à la 2ème itération réactive sur le même domaine (deploy, performance, data shape, etc.), STOP immédiat des patches symptomatiques et passer en audit EXHAUSTIF (inventaire complet + verif présence chaque dep + test E2E scenario réel local + risques résiduels documentés). La 2ème itération réactive est le signal qu'on est dans une boucle d'enfer ; chaque patch suivant a un ROI négatif vs un audit exhaustif initial.

### Test E2E scenario UTILISATEUR avant claim "prêt prod" (s32)
S32 : 5 fois j'ai claim « prêt prod » et 4 fois ça a cassé en prod sur des choses non testées (build OK ≠ deploy OK, deploy OK ≠ pipeline PDF OK, pipeline PDF OK ≠ workflow étape 3→4 OK). Pattern persistant : tester juste le build / le boot / un endpoint isolé n'est PAS suffisant pour valider une livraison.
**Règle qa + reviewer + orchestrator** : avant tout claim « prêt prod », tester en local le scenario UTILISATEUR exact (workflow complet, pas isolé) avec données représentatives (rows pre-migration incluses). Format : `curl /workflow/end-point` doit reproduire le scenario que l'utilisateur va faire, pas juste un health check. Si le scenario réel n'est pas testable en local : explicitement le dire (« non vérifiable hors smoke test prod ») plutôt que claim faux.

### Workflow architecte = pills structurés + chat conversationnel COMPLÉMENT (s32)
Verbatim Thomas s32 : *« il faut que ce soit les 2 : toutes les infos standards champs ET puis questions si besoins de choses en plus »*. Refus du pattern « tout par chat » (lent, conversation longue) ET refus du pattern « tout par pills » (rigide, manque les nuances). Le bon workflow est hybride : pills systématiques pour les infos structurées (saisie rapide 30s/pièce), chat conversationnel par-dessus pour les ambiguïtés non couvertes (intentions travaux, contraintes spécifiques).
**Règle product-manager + ux + ia** : pour tout outil de saisie d'information complexe vers IA, structurer en 2 couches : (1) pills/forms pour les données catégorielles (style, type, niveau, etc.), (2) chat optionnel pour le complément libre. Le chat ne doit JAMAIS re-poser une question dont la réponse est dans les pills (discipline LLM strict via system prompt avec liste explicite des champs déjà renseignés + règle absolue).

### Saisie marchand > inférence IA (priorité absolue dealer-confirmed) (s32)
Verbatim persona Thomas s32 : *« Si je dis "Sol à rénover" c'est parce que j'ai décidé de présenter la pièce en version rénovée dans le rendu. La photo source peut montrer un parquet correct mais je veux vendre la version après travaux. C'est précisément pour ça que j'utilise l'outil. L'IA qui "corrige" ma saisie parce qu'elle pense voir mieux que moi = outil inutilisable. »* La règle : la saisie marchand a TOUJOURS priorité sur l'inférence Vision/LLM, jamais l'inverse, sans exception. Le marchand vend une INTENTION (la version après travaux), pas un constat (l'état actuel).
**Règle ia + fullstack + ux** : pour tout pipeline qui combine saisie utilisateur + inférence IA, implémenter (1) tag explicite source dans le prompt (`(dealer-confirmed)` vs `(visually identified)`), (2) guardrail backend qui interdit aux tools IA d'écraser un champ user-confirmed (return error tool si tentative), (3) UI qui distingue visuellement les valeurs confirmées (couleur normale) des valeurs détectées par IA en attente de validation (couleur orange + badge "À confirmer"). Validation explicite par l'utilisateur (clic) requise avant que la valeur Vision soit considérée "confirmée".
